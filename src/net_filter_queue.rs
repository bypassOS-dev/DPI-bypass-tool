...
pub fn start_sniff() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let mut sequence_storage: Vec<u32> = Vec::new();
        ...
        if let Some(ip_packet) = Ipv4Packet::new(payload) {
            if let Some(tcp_packet) = TcpPacket::new(ip_packet.payload()) {
                ...
                if tcp_payload.len() > 5 && tcp_payload[0] == 0x16 {
                    ...
                    for &seq_vec in &sequence_storage {
                        if seq_vec == seq {
                            is_here = 1;
                        }
                    }
                    if is_here != 1 {
                        sequence_storage.push(seq);
                    ...
                    if let Some((split_pos, domain)) = find_sni(tcp_payload) {
                        ...
                        let my_ip =
                            SocketAddrV4::new(ip_packet.get_source(), tcp_packet.get_source());
